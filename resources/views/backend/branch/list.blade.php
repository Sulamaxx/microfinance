        ...
        <div class="col-lg-12">
            <div class="card no-export">
                <div class="card-header d-flex align-items-center">
                    <span class="panel-title">{{ _lang('All Center') }}</span>
                    <a class="btn btn-primary btn-xs ml-auto ajax-modal" data-title="{{ _lang('Add New Center') }}"
                        href="{{ route('branches.create') }}"><i class="ti-plus"></i>&nbsp;{{ _lang('Add New') }}</a>
                </div>
                <div class="card-body">
                        ...
                        <tbody>
                            @foreach ($branchs as $branch)
                                <tr data-id="row_{{ $branch->id }}">
                                    ...
                                    <td class='no'>{{ $branch->no }}</td>
                                    <td class='code'>{{ $branch->code }}</td>
                                    <td class='weekdays'>
                                            ...
                                            <form action="{{ route('branches.destroy', $branch['id']) }}" method="post">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                    <a href="{{ route('branches.edit', $branch['id']) }}"
                                                        data-title="{{ _lang('Update Center') }}"
                                                        class="dropdown-item dropdown-edit ajax-modal"><i
                                                            class="ti-pencil-alt"></i>&nbsp;{{ _lang('Edit') }}</a>
                                                    <a href="{{ route('branches.show', $branch['id']) }}"
                                                        data-title="{{ _lang('Center Details') }}"
                                                        class="dropdown-item dropdown-view ajax-modal"><i
